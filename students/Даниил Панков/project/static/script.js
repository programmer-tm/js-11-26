...
Vue.component("search-good", {
    ...
    template: `
   <input
   v-on:input="newsearch" type="text" class="search" placeholder="Поиск товара" />
   `,
    methods: {
        newsearch(input) {
...
Vue.component("v-item", {
    ...
    template: `
     ...
     <button type=" button" v-on:click="removeItem(element.id_product)">Удалить из корзины</button>
     </div>
   `,
    methods: {
        addToBasket() {
        ...
        fetchBasketData() {
            return new Promise((resolve, reject) => {
                sendRequest("basket")
                    ...
                    .catch((error) => {
                        this.errorMessage = "Корзина не заргужена";
                    });
            });
        },
        addToBasket(item) {
